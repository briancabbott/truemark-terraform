...
locals = {
  'name': name,
  'provisioner_name': f"${name}-provisioner",
  'sam_s3_bucket': f"${data_awsci.current.account_id}-${name}-sam",
...
Resource("aws_iam_user_policy_attachment", "s3", 
    policy_arn = mod_s3.iam_policy_rw_arn, 
    user = data_awsiam.sam.user_name)